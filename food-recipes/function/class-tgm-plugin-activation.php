<?php if ( ! class_exists( 'foodrecipes_Plugin_Activation' ) ) {
    /*  Automatic plugin installation and activation library. */
    class foodrecipes_Plugin_Activation {
        /* Holds a copy of itself, so it can be referenced by the class name. */
        public static $instance;
        /* Holds arrays of plugin details. */
        public $plugins = array();
        /* Name of the unique ID to hash notices. */
        public $id = 'foodrecipes';
        /* Name of the querystring argument for the admin page. */
        public $menu = 'foodrecipes-install-plugins';
        ...
        public function __construct() {
            ...
            do_action_ref_array( 'foodrecipes_init', array( $this ) );
        ...
        public function thickbox() {
            if ( ! get_user_meta( get_current_user_id(), 'foodrecipes_dismissed_notice_' . $this->id, true ) ) {
                add_thickbox();
            } }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new foodrecipesPA_List_Table;
            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'foodrecipes-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            } ?>
<div class="foodrecipes wrap">
  ...
  <form id="foodrecipes-plugins" action="" method="post">
    <input type="hidden" name="foodrecipes-page" value="<?php echo $this->menu; ?>" />
...
<?php
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['foodrecipes-install'] ) && 'install-plugin' == $_GET['foodrecipes-install'] ) ) {
                check_admin_referer( 'foodrecipes-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'foodrecipes-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'foodrecipes-install' ); // Extra fields to pass to WP_Filesystem.
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['foodrecipes-activate'] ) && 'activate-plugin' == $_GET['foodrecipes-activate'] ) ) {
                check_admin_referer( 'foodrecipes-activate', 'foodrecipes-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( $this->is_foodrecipes_page() ) { return; }
            // Return early if the nag message has been dismissed.
            if ( get_user_meta( get_current_user_id(), 'foodrecipes_dismissed_notice_' . $this->id, true ) ) {  return; }
            ...
            if ( ! empty( $message ) ) {
                ...
                $action_links = apply_filters(
                    ...
                    array(
                        ...
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'foodrecipes-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'foodrecipes', 'foodrecipes', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'foodrecipes', 'foodrecipes', $rendered, $nag_class );
                } }
            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'foodrecipes' );
            } }
        /* Add dismissable admin notices. */
        public function dismiss() {
            if ( isset( $_GET['foodrecipes-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'foodrecipes_dismissed_notice_' . $this->id, 1 );
            } }
        ...
        public function actions( $install_actions ) {
            // Remove action links on the foodrecipesPA install page.
            if ( $this->is_foodrecipes_page() ) { return false; }
        ...
        /* Determine if we're on the foodrecipesPA Install page. */
        protected function is_foodrecipes_page() {
        ...
        public function update_dismiss() {
            delete_user_meta( get_current_user_id(), 'foodrecipes_dismissed_notice_' . $this->id );
        }
        ...
        public static function get_instance() {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof foodrecipes_Plugin_Activation ) ) {
                self::$instance = new foodrecipes_Plugin_Activation();
            } return self::$instance;
        } }
    // Ensure only one instance of the class is ever invoked.
    $foodrecipes = foodrecipes_Plugin_Activation::get_instance();
}
if ( ! function_exists( 'foodrecipes' ) ) {
    /* Helper function to register a collection of required plugins. */
    function foodrecipes( $plugins, $config = array() ) {
        foreach ( $plugins as $plugin ) {
            foodrecipes_Plugin_Activation::$instance->register( $plugin );
        } if ( $config ) {
            foodrecipes_Plugin_Activation::$instance->config( $config );
        } } }
/* WP_List_Table isn't always available. If it isn't available, we load it here. */
if ( ! class_exists( 'WP_List_Table' ) ) { require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}
if ( ! class_exists( 'foodrecipesPA_List_Table' ) ) {
    /* List table class for handling plugins. */
    class foodrecipesPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foodrecipes_Plugin_Activation::$instance->admin_init();
            foodrecipes_Plugin_Activation::$instance->thickbox();
            // Prep variables for use and grab list of all installed plugins.
            $table_data        = array();
            $i                 = 0;
            $installed_plugins = get_plugins();
            foreach ( foodrecipes_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
        ...
        /* Retrieve plugin data, given the plugin name. Taken from the foodrecipes_Plugin_Activation class.*/
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            foreach ( foodrecipes_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                } } return false;
        }
                    ...
                    'activate' => sprintf(
                        ...
                        add_query_arg(
                            array(
                                ...
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'foodrecipes-activate'       => 'activate-plugin',
                                'foodrecipes-activate-nonce' => wp_create_nonce( 'foodrecipes-activate' ),
                            ), admin_url( 'themes.php' )
                        ), $item['sanitized_plugin']
                    ), );
            }
        ...
        public function process_bulk_actions() {
            ...
            if ( 'foodrecipes-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => foodrecipes_Plugin_Activation::$instance->menu,
                            'foodrecipes-action'  => 'install-selected',
                ...
                $sources      = array();
                $install_path = array();
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'foodrecipes-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'foodrecipes-empty';
                    $i++;
                }
                if ( is_wp_error( $api ) ) { wp_die( foodrecipes_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) ); }
                ...
                $url   = add_query_arg( array( 'page' => foodrecipes_Plugin_Activation::$instance->menu ), admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
                // Create a new instance of foodrecipes_Bulk_Installer.
                $installer = new foodrecipes_Bulk_Installer( $skin = new foodrecipes_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );
                // Wrap the install process with the appropriate HTML.
                echo '<div class="foodrecipes wrap">';
                    if ( version_compare( foodrecipes_Plugin_Activation::$instance->wp_version, '3.8', '<' ) ) {
                    }
            ...
            if ( 'foodrecipes-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
 ...
 */
add_action( 'admin_init', 'foodrecipes_load_bulk_installer' );
function foodrecipes_load_bulk_installer() {
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && foodrecipes_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
        if ( ! class_exists( 'foodrecipes_Bulk_Installer' ) ) {
            /* Installer class to handle bulk plugin installations. */
            class foodrecipes_Bulk_Installer extends WP_Upgrader {
                ...
                public function bulk_install( $packages ) {
                    ...
                    if ( foodrecipes_Plugin_Activation::$instance->is_automatic ) { $this->activate_strings(); }
                ...
                public function run( $options ) {
                    ...
                    if ( foodrecipes_Plugin_Activation::$instance->is_automatic ) {
                        ...
                        foodrecipes_Plugin_Activation::$instance->populate_file_path();
                        // Set correct strings based on results.
                        if ( is_wp_error( $activate ) ) {
        ...
        if ( ! class_exists( 'foodrecipes_Bulk_Installer_Skin' ) ) {
            /* Installer skin to set strings for the bulk plugin installations. */
            class foodrecipes_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function bulk_footer() {
                    ...
                    foreach ( foodrecipes_Plugin_Activation::$instance->plugins as $plugin ) {
                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                            echo '<p><a href="' . add_query_arg( 'page', foodrecipes_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '" title="' . esc_attr( foodrecipes_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . foodrecipes_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                            $complete[] = $plugin;
                            break;
                        } else {
                            $complete[] = '';
